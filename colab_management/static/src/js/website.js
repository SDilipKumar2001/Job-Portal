...
function checkSpcialChar(event){
 ...
 function GetDays(fromField, toField) {
    var date1 = new Date(fromField.value);
    var date2 = new Date(toField.value);
    var Difference_In_Time = date2.getTime() - date1.getTime();
    var Difference_In_Days = Math.ceil(Difference_In_Time / (1000 * 3600 * 24));
    var years = Math.floor(Difference_In_Days / 365);
    var months = Math.floor(Difference_In_Days % 365 / 30);
    var days = Math.floor(Difference_In_Days % 365 % 30);
    return years + "y" + " " + months + "m" + " " + days + "d";
}

function caller(row) {
    var fromField = row.querySelector(".from_field");
    var toField = row.querySelector(".to_field");
    var yearsField = row.querySelector(".years");

    if (fromField && toField && yearsField) {
        yearsField.value = GetDays(fromField, toField);
    }
}
//  function cal() {
